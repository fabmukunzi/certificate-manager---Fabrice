  ...
  public CertificateDto createCertificate(CertificateDto certificateDto) {
    ...
    if (certificateDto.getSupplier().getId() != null) {
      ...
      throw new EntityNotFoundException(
          "Supplier with ID " + certificateDto.getSupplier().getId() + " is not found");
    }
    if (certificateDto.getAssignedUsers() != null) {
      List<UserEntity> users =
          certificateDto.getAssignedUsers().stream()
              ...
              .map(
                  commentDto -> {
                    return commentsToRemove.stream()
                        .filter(
                            existingComment -> existingComment.getId().equals(commentDto.getId()))
                        .findFirst()
                        .map(
                            existingComment -> {
                              CommentMapper.toEntity(commentDto, certificateEntity, userRepository);
                        ...
                        .orElseGet(
                            () ->
                                CommentMapper.toEntity(
                                    commentDto, certificateEntity, userRepository));
                  })
              .collect(Collectors.toList());
