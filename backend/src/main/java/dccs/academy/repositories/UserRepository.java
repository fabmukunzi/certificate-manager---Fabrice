...
public class UserRepository implements PanacheRepository<UserEntity> {
  ...
  public List<UserDto> userSearch(
      String firstName, String lastName, String plant, String userId, String department) {
    CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
    var query = cb.createQuery(UserEntity.class);
    var root = query.from(UserEntity.class);
    List<Predicate> predicates = new ArrayList<>();
    predicates.add(
        PredicateUtils.createLikePredicate(cb, root.get(UserEntity_.FIRST_NAME), firstName));
    predicates.add(
        PredicateUtils.createLikePredicate(cb, root.get(UserEntity_.LAST_NAME), lastName));
    ...
    if (department != null) {
      ...
      predicates.add(
          PredicateUtils.createLikePredicate(
              cb, departmentJoin.get(DepartmentEntity_.TITLE), department));
    }
    query
        .select(root)
        .where(cb.and(predicates.stream().filter(Objects::nonNull).toArray(Predicate[]::new)));
